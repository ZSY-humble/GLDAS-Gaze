...
from common.losses import FixationKLLoss, CombinedFixationLoss
...
def parse_args():
    ...
    parser.add_argument('--hparams', type=str, help='Path to hparams JSON')
    ...
    parser.add_argument('--transfer-learn', choices=['none', 'finetune'], default='none')
    parser.add_argument('--eval-only', action='store_true', help='Evaluation only')
    parser.add_argument('--split', type=int, default=1, help='Dataset split id')
...
def compute_loss(model, batch, losses, loss_funcs, pa):
    ...
    if "next_fix_pred" in losses:
        ...
        if use_focal_loss and not (use_combined_loss or use_kl_loss):
...
if __name__ == '__main__':
    ...
    if args.eval_only:
        ...
        use_combined_loss = hparams.Train.get('use_combined_loss', False)
        use_kl_loss = hparams.Train.get('use_kl_loss', False)
        if use_combined_loss:
            next_fix_loss = CombinedFixationLoss(
                bce_weight=hparams.Train.get('bce_weight', 1),
                kl_weight=hparams.Train.get('kl_weight', 1),
                use_focal=use_focal_loss
            )
        elif use_kl_loss:
            next_fix_loss = FixationKLLoss(
                temperature=hparams.Train.get('kl_temperature', 1.0),
                scale=hparams.Train.get('kl_scale', 1.0)
            )
        else:
            next_fix_loss = focal_loss if use_focal_loss else torch.nn.BCEWithLogitsLoss()
        loss_funcs = {
            "next_fix_pred": next_fix_loss,
            ...
            "mismatch_cls": torch.nn.BCEWithLogitsLoss(),
            "term_pred": torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(term_pos_weight, dtype=torch.float32)),
        }
        ...
        for i_epoch in range(s_epoch, int(1e5)):
            ...
            for i_batch, batch in enumerate(train_gaze_loader):
                ...
                if global_step % print_every == print_every - 1:
                    ...
                    eta = str((time - last_time) / print_every * (max_iters - global_step))
                    last_time = time
                    time = str(time)
                ...
                if (global_step >= eval_start_step and global_step % eval_every == eval_every - 1) or \
   (10000 <= global_step <= 22000 and global_step % 250 == 0):
                    if args.pretrain:
                ...
                if (global_step >= eval_start_step and global_step % save_every == save_every - 1) or \
   (10000 <= global_step <= 22000 and global_step % 250 == 0):
